...
// ── MAC login feedback — 2 toasts: success ✅ / fail ❌ ─────────────────────
(function() {
  if (!document.getElementById('_em-style')) {
    const s = document.createElement('style');
    s.id = '_em-style';
    s.textContent = `
      @keyframes _emIn  { from{transform:translateX(-50%) translateY(10px);opacity:0} to{transform:translateX(-50%) translateY(0);opacity:1} }
      @keyframes _emOut { from{opacity:1;transform:translateX(-50%) translateY(0)} to{opacity:0;transform:translateX(-50%) translateY(6px)} }
      ._em-toast {
        position:fixed; bottom:24px; left:50%; transform:translateX(-50%) translateY(10px);
        opacity:0; z-index:9999; pointer-events:none; display:flex; align-items:center; gap:8px;
        background:rgba(18,20,28,0.93); color:#fff; border-radius:10px;
        padding:9px 16px; font-size:.82rem; font-weight:600; letter-spacing:.01em;
        box-shadow:0 4px 20px rgba(0,0,0,0.3); backdrop-filter:blur(10px);
        font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;
        transition:none; white-space:nowrap;
      }
      ._em-toast.in  { animation:_emIn .18s ease forwards; }
      ._em-toast.out { animation:_emOut .15s ease forwards; }
    `;
    document.head.appendChild(s);
  }

  const _CFG = {
    login_success: { emoji: '✅', label: 'Welcome back!',       accent: '#22c55e' },
    login_fail:    { emoji: '❌', label: 'Invalid credentials', accent: '#ef4444' },
    conn_error:    { emoji: '❌', label: 'Connection error',    accent: '#f59e0b' },
    error:         { emoji: '❌', label: 'Something went wrong',accent: '#f97316' },
  };
  ...
  window.showEmojiMoment = function(type) {
    ...
    el.innerHTML = `<span>${cfg.emoji}</span><span style="color:${cfg.accent}">${cfg.label}</span>`;
    ...
    setTimeout(() => {
      ...
      setTimeout(() => { if (el.parentNode) el.remove(); if (_active === el) _active = null; }, 160);
    }, type === 'login_success' ? 900 : 1800);
  };
}());
